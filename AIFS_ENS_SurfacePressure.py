...
# ECMWF AIFS-ENS コントロールメンバー（cf）地上気圧・風・2m気温天気図描画スクリプト
# ECMWF AI-Integrated Forecast System アンサンブル版のコントロールメンバーを描画する。
# データ取得: ECMWF Open Data（最新5日分）
# 20260619 上原政博
#
# AIFS-ENSについて:
#   - 00/06/12/18z 全初期時刻で利用可能（aifs-singleと異なり6z/18zも提供）
#   - cf（コントロールメンバー）: ~85MB/FT
#   - pf（全摂動メンバー統合）: ~4GB/FT（本スクリプトでは対象外）
#   - FT最大360h、6h間隔
#   - ECMWF Open Data URL: https://data.ecmwf.int/forecasts/{YYYYMMDD}/{HH}z/aifs-ens/0p25/enfo/
#
# 使用例:
#   python AIFS_ENS_SurfacePressure.py 2026061812 0 1          # FT=0h 1枚
#   python AIFS_ENS_SurfacePressure.py 2026061812 0 5          # FT=0,6,12,18,24h 5枚
#   python AIFS_ENS_SurfacePressure.py 2026061812 0 1 --area 108 156 5 45
...
AIFS_ENS_BASE_URL = "https://data.ecmwf.int/forecasts"
HEADERS   = {"User-Agent": "Mozilla/5.0 (compatible; AIFS-ENS-Downloader/1.0)"}
DATA_DIR  = "./data/aifs-ens"


def ensure_file_aifs_ens(path, fn, year, month, day, hour):
    """AIFS-ENS cfファイルを確認し、なければダウンロードする。"""
    if os.path.exists(path):
        return True
    print(f"AIFS-ENSデータファイルが見つかりません: {fn}")
    print("ECMWF Open Data (AIFS-ENS cf) からダウンロードを試みます（最新5日分のみ利用可）...")
    url = f"{AIFS_ENS_BASE_URL}/{year:04d}{month:02d}{day:02d}/{hour:02d}z/aifs-ens/0p25/enfo/{fn}"
    dest = Path(path)
    ...
    try:
        with requests.get(url, headers=HEADERS, stream=True, timeout=180) as r:
            ...
            print()
        print(f"ダウンロード完了: {fn} ({dest.stat().st_size/1048576:.1f} MB)")
        return True
    except requests.HTTPError as e:
        print(f"\nダウンロード失敗（HTTP {e.response.status_code}）")
        if e.response.status_code == 404:
            print("  AIFS-ENSデータが存在しません。最新5日分のみ利用可能です。")
        if dest.exists():
            dest.unlink()
        return False
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='ECMWF AIFS-ENS cfファイルから地上気圧・風・2m気温天気図を描画する',
...
使用例:
  ...
  # 描画範囲・平滑化・矢羽間隔の指定:
  python AIFS_ENS_SurfacePressure.py 2026061812 0 1 --area 108 156 5 45
  python AIFS_ENS_SurfacePressure.py 2026061812 0 1 --smooth-size 15
  python AIFS_ENS_SurfacePressure.py 2026061812 0 1 --wind-step 10

デフォルト描画設定:
  --area        108 156 17 55  東経108〜156°、北緯17〜55°
  --smooth-size 10             10×10格子平均スムージング
  --wind-step   5              風矢羽を5格子おき

AIFS-ENSについて:
  ECMWF AIFSアンサンブルのコントロールメンバー（cf）
  00/06/12/18z 全初期時刻で利用可能（~85MB/FT）
  FT最大360h、6h間隔

実行環境（conda の場合）:
  conda activate met_env_310
  python AIFS_ENS_SurfacePressure.py [引数]
"""
    )
...
def plot_one(i_year, i_month, i_day, i_hourZ, ft_hours, output_dir, area=None, smooth_size=10, wind_step=5):
    fn   = f"{i_year:04d}{i_month:02d}{i_day:02d}{i_hourZ:02d}0000-{ft_hours:d}h-enfo-cf.grib2"
    path = f"{DATA_DIR}/{fn}"

    if not ensure_file_aifs_ens(path, fn, i_year, i_month, i_day, i_hourZ):
        print(f"スキップ: FT={ft_hours}h（データ取得失敗）")
        return False

    print(f"[{ft_hours:4d}h] データ読み込み: {fn}")
    ...
    title = f"AIFS-ENS(cf) FT{ft_hours:d}h IT:{dt_str} Surface Pre, Wind, Temp"
    fig.text(0.5, 0.01, title, ha='center', va='bottom', size=15)

    os.makedirs(output_dir, exist_ok=True)
    out_fn = f"{output_dir}/{dt_str2}_FT{ft_hours:03d}h_AIFS_ENS_SurfacePressure.png"
...
def main():
    ...
    print(f"初期時刻: {init_str} UTC  AIFS-ENS(cf) 地上気圧・風・2m気温")
