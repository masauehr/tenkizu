...
#   python typhoon-multi.py 2026052712 0000 3 --interval 12               # 12h間隔 3枚
#   python typhoon-multi.py 2026052712 0000 12h                           # 12hプリセット
...
DEFAULT_AREA = [108, 156, 5, 45]   # 東経108〜156°、北緯5〜45°
...
描画範囲の指定例（--area LON_W LON_E LAT_S LAT_N）:
  --area 108 156   5  45   東アジア（デフォルト）
  --area 100 170   0  50   広域西太平洋
  --area 120 150  15  40   日本近海
  --area 120 140  15  35   沖縄周辺
...
"""
    ...
    parser.add_argument("--area", type=float, nargs=4, default=DEFAULT_AREA,
                        metavar=("LON_W", "LON_E", "LAT_S", "LAT_N"),
                        help=f"描画範囲 経度西 経度東 緯度南 緯度北（デフォルト: {' '.join(map(str, DEFAULT_AREA))}）")
...
def main():
    ...
    area     = args.area
    area_str = " ".join(str(int(v) if v == int(v) else v) for v in area)
    area_arg = f"--area {area_str}"
    ...
    print(f" 描画範囲: LON {area[0]}〜{area[1]}°E  LAT {area[2]}〜{area[3]}°N")
    print("=" * 60)
    ...
    for ft_h in ft_list_h:
        ...
        if run_gsm:
            ok = run_python("GSM_faxSrfPre.py",
                            f"{init_str} {ft_ddhh:04d} 1 {area_arg}",
                            str(script_dir))
            if not ok:
        ...
        if run_gfs:
            ok = run_python("GFS_SurfacePressure.py",
                            f"{init_str} {ft_h} 1 {area_arg} --smooth-size 5 --wind-step 10",
                            str(script_dir))
            if not ok:
    ...
    lines = [
        ...
        "",
        f"**初期時刻**: {dt_display}",
        f"**描画範囲**: LON {area[0]}〜{area[1]}°E  LAT {area[2]}〜{area[3]}°N",
        "",
        "---",
        "",
        "## 地上気圧・10m風・2m気温",
