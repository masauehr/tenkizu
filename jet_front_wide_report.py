...
def main():
    ...
    if avg_steps > 1:
        end_ft_h = start_ft_h + (n_steps - 1) * 6 * avg_steps + (avg_steps - 1) * 6
        ft_label = f"FT{start_ft_h}-{end_ft_h}_avg{avg_steps}"
    else:
        end_ft_h = start_ft_h + (n_steps - 1) * 6
        ft_label = f"FT{start_ft_h}" if n_steps == 1 else f"FT{start_ft_h}-{end_ft_h}"
    ...
    avg_info = f"  平均: {avg_steps}ステップ/枚" if avg_steps > 1 else ""
    print(f"{'='*60}")
    ...
    if avg_steps > 1:
        ...
        batch_starts = [start_ft_h + i * 6 for i in range(n_steps)]
