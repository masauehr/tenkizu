...
| モデル | 運営機関 | 無償GRIB2 | リアルタイム | 取得可能期間 | 実装 | 備考 |
|--------|---------|:---------:|:-----------:|------------|:----:|------|
| GSM | 気象庁（日本） | ✓ | ✓ | 全期間 | ✓ | RISH京大 |
| ECMWF | 欧州中期予報センター | ✓ | ✓ | 最新約5日分 | ✓ | ECMWF Open Data |
| GFS | NOAA/NCEP（米国） | ✓ | ✓ | 最新約10日分 | ✓ | NOMADS filter |
| JRA-55 | 気象庁（再解析） | ✓（認証必要） | ✗ | 1958年〜 | ✓ | RISH京大・要ID登録 |
| UKMET UM | 英国気象庁 | ✗ | — | — | ✗ | 商用ライセンスのみ |
| GEM | カナダ気象局 | ✓ | ✓ | 数日分 | ✗ | MSC Datamart |
| CFS | NOAA/NCEP（季節） | ✓ | ✓ | — | ✗ | NOMADS・季節予報用途 |
